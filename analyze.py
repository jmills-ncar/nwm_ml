...
class AnalysisDataset(object):
    """A dataset consisting of a modeled and observed dataframe. This class provides methods for
    calculating staistics and making basic exploratory plots. The two datasets are joined using
    an inner join"""
    ...
    def __init__(
        ...
        Args:
            ...
            join_how: Optional, how to perform teh dataframe join. Default is 'inner'. Options
            are 'inner','left','right'.
        """
        if join_on is None:
            join_on = ['feature_id', 'time']

        self.data = pd.merge(modeled_df,
                             observed_df,
                             on=join_on,
                             how=join_how,
                             suffixes=['_mod',
    ...
    def _calc_cont_stats(data,
        ...
        if len(data[threshold].dropna()) > 0:
            ...
            cont_stats = calc_cont_stats(cont_table, inf_as_na=inf_as_na, decimals=decimals)
    ...
    @staticmethod
    def _calc_gof_stats(data,
                        obs_col,
                        mod_col,
                        inf_as_na,
                        decimals):
        return calc_gof_stats(data[obs_col],
                              data[mod_col],
                              inf_as_na=inf_as_na,
                              decimals=decimals
                              )
    ...
    def _calc_flow_metrics(data,
        ...
        if len(data[threshold].dropna()) > 0:
            ...
            flow_metrics = calc_flow_metrics(
                observed,
                modeled,
                decimals=decimals
                )
            if label is None:
                label = threshold
            flow_metrics['threshold'] = label
            flow_metrics['value'] = flow_metrics['value'].round(decimals=decimals)
            return flow_metrics
        else:
            if label is None:
                label = threshold
    ...
    def calc_cont_stats(self,
        ...
        Probability of false detection (pofd, false alarm rate) = false_alarms/(false_alarms +
        correct_negatives)
        Success ratio (sr) = hits/(hits + false_alarms)
        Critical success index (csi, threat score) = hits/(hits + misses + false_alarms)
        Gilbert skill score (gss, Equitable threat score) = (hits - hits_random)/(hits + misses +
        false_alarms - hits_random) where hits_random = ((hits + misses)*(hits +
        false_alarms))/total
        Hanssen and Kuipers discriminant (hk, true skill statistic, Peirce's skill score) = (hits/(
        ...
        (misses * false_alarms))/((hits * correct_negatives) + (misses * false_alarms)

        Args:
            threshold: The threshold value for contingency stats or column name in self.data
            containing threshold value. The first value of the column will be used as the
            threshold value.
            label: Label for the threshold, value used if None.
            time_window: Calculate contingency statistics over a moving
            time window of specified width in seconds ('s'), hours ('h'), or days('d').
        ...
        if group_by:
            cont_stats = self.data.set_index(group_by).groupby(group_by). \
                apply(self._calc_cont_stats,threshold=threshold,
                      label=label,
            ...
            cont_stats = self._calc_cont_stats(data=self.data,
                                               threshold=threshold,
                                               label=label,
                                               time_window=time_window,
                                               mod_col=mod_col,
    ...
    def calc_gof_stats(self,
        ...
        See :py:fun:`calculate_all_functions() <calculate_all_functions>` in :py:mod:`spotpy`.
        Args:
            mod_col: Column name of modelled data
            obs_col: Column name of observed data
            group_by: Column names to group by prior to calculating statistics
        ...
        if group_by is None:
            gof_stats = self._calc_gof_stats(data=self.data,
                                             obs_col=obs_col,
                                             mod_col=mod_col,
                                             inf_as_na=inf_as_na,
                                             decimals=decimals)
    ...
    def calc_flow_metrics(self,
        ...
        if group_by is None:
            flow_metrics = self._calc_flow_metrics(data=self.data,
                                                   threshold=threshold,
                                                   label=label,
                                                   obs_col=obs_col,
                                             mod_col=mod_col,
            ...
            flow_metrics = self.data.set_index(group_by). \
                groupby(group_by). \
                apply(self._calc_flow_metrics,
                      threshold=threshold,
                      label=label,
                      obs_col=obs_col,
                      mod_col=mod_col,
                      decimals=decimals)
            flow_metrics = flow_metrics.rename(columns={'level_2': 'statistic'})
...
def calc_cont_table(actual: np.array, predicted: np.array) -> pd.DataFrame:
    """
    Calculate a contingency table from two arrays of hits/misses. In each input array, a hit is True
    and a miss is a False
    ...
    # Have to manually construct table because pandas will drop columns on return table if all
    # values are 0
    try:
...
def calc_cont_stats(cont_table: pd.DataFrame,
                    inf_as_na: bool = True,
                    decimals: int = 2):
    """
    ...
    Critical success index (csi, threat score) = hits/(hits + misses + false_alarms)
    Gilbert skill score (gss, Equitable threat score) = (hits - hits_random)/(hits + misses +
    false_alarms - hits_random) where hits_random = ((hits + misses)*(hits + false_alarms))/total
    Hanssen and Kuipers discriminant (hk, true skill statistic, Peirce's skill score) = (hits/(
    hits+misses)) - (false_alarms/(false_alarms + correct_negatives))
    Odds ratio (or) = (POD/(1-POD))/(POFD/(1-POFD))
    Odds ratio skill score (orss, Yule's Q) = ((hits * correct_negatives) -
    ...
    cont_stats = [
        ...
        ('gss', (hits - hits_random) / (hits + misses + false_alarms - hits_random)),
        ('hk', (hits / (hits + misses)) - (false_alarms / (false_alarms + correct_neg))),
    ...
    cont_stats = pd.DataFrame(cont_stats).rename(columns={0: 'statistic', 1: 'value'})
...
def calc_gof_stats(actual: np.array, predicted: np.array,
                   inf_as_na:bool = True, decimals: int = 2):
    """
    Calculate goodness of fit statistics using the spotpy package.
    See :py:fun:`calculate_all_functions() <calculate_all_functions>` in :py:mod:`spotpy`.
    Args:
    ...
    gof_stats = pd.DataFrame(gof_stats).rename(columns={0: 'statistic', 1: 'value'})

    # Change the sign on bias since it is presented in spotpy as obs-sim instead of typical sim-obs
    gof_stats.loc[gof_stats['statistic'] == 'bias',['value']] = \
        gof_stats.loc[gof_stats['statistic'] == 'bias', ['value']] * -1

    #Screen out very large numbers so that dont have to move to double precision
    ...
    # Summarize actual
    summary = pd.DataFrame({'value': [actual.mean(), np.percentile(actual, 0.5), actual.std()],
...
def calc_flow_metrics(actual: np.array,
                      predicted: np.array,
                      decimals: int = 2):
    """
    Calculate goodness of fit statistics using the spotpy package.
    ...
    df = pd.DataFrame({'statistic': ['event_freq_bias',
                                    'event_dur_bias',
                                    'N_obs_events'],
                       'value':[event_freq_bias,
                                event_dur_bias,
                                num_act_events]})
    return df
